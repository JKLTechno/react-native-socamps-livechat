...
import NavigationBar from './SOCAmpsNavigation';

const { height, width } = Dimensions.get('window');
const totalSize = num => (Math.sqrt((height * height) + (width * width)) * num) / 100;

export default class SOCAmpsChat extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      messages: [],
    ...
    this.handleAgentChanged.bind(this);
    this.handleStateChange.bind(this);
    this.handleTypingIndicator.bind(this);
    this.handleChatEnded.bind(this);
    this.hendleVisitorData.bind(this);
